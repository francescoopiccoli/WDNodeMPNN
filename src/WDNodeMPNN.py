...
class WDNodeMPNN(nn.Module):
    ...
    def forward(self, data):
        ...
        incoming_edge_weighted_sum = torch.zeros(x.size()[0], edge_attr.size()[1])
        ...
        concat_features = torch.cat([x, incoming_edge_weighted_sum], dim=1)
        ...
        # atom_weight to use stoichiometry
...
class MessagePassingLayer(MessagePassing):
    ...
    # Constructs messages to node i ifor each edge (j,i) if flow="source_to_target"
